...
import CompanySidebarNav from "@/features/company-dashboard/components/CompanySidebarNav";

export default function CompanyLayoutClient({ children, statusBadge }) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 transform border-r border-white/30 bg-white/80 px-6 py-8 shadow-2xl backdrop-blur-2xl transition-transform duration-300 ease-in-out md:hidden ${
      ...
      >
        ...
        <CompanySidebarNav onItemClick={closeMobileMenu} />
      </aside>

      {/* Desktop Sidebar */}
      <aside className="hidden w-56 flex-col border-r border-white/30 bg-white/40 px-6 py-8 shadow-[10px_0_40px_rgb(0,0,0,0.02)] backdrop-blur-2xl md:flex">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="hidden mt-1 md:flex items-center text-sm font-medium text-slate-500">
                Manage your events and track candidate engagement.
              </p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            {/* Status badge injected from server */}
            {statusBadge}
          </div>
        </header>
